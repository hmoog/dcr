...
\end{enumerate}
The macro-configuration space $\tilde{\Omega}$ has dimension at most
$\dim\Omega$, with strict inequality whenever any $g_{s'}$ is
non-injective---the generic and intended case. Concrete choices of
...
\begin{enumerate}[nosep]
  \item \textbf{Fast intra-group mixing.} For each macro-component
    $s' \in S'$ and each macro-state
    $\tilde{\omega}_{s'} \in \tilde{D}_{s'}$, the intra-group dynamics
    (freezing all components outside the group) admits a unique conditional
    equilibrium $\nu_{s'}(\cdot \mid \tilde{\omega}_{s'})$ on the fast
    degrees of freedom---those not captured by $g_{s'}$---and mixes to
    this equilibrium in time $\tau_{\text{fast}}$.
  ...
  \item \textbf{Macro-sufficiency.} For each inter-group edge
    $\{s_1, s_2\} \in E_{\text{cross}}$ with $\pi(s_j) = s'_j$, the
    expected micro-violation under the conditional equilibria,
    $\mathbb{E}_{\nu_{s'_1}(\cdot \mid \tilde{\omega}_{s'_1}) \otimes
    \nu_{s'_2}(\cdot \mid \tilde{\omega}_{s'_2})}
    [v_{\{s_1,s_2\}}]$,
    is determined by the macro-states
    $(\tilde{\omega}_{s'_1}, \tilde{\omega}_{s'_2})$ alone.
\end{enumerate}
Condition~4 says that the compression maps capture all information
relevant to inter-group interactions: once the fast degrees of freedom
have equilibrated, the macro-state is a sufficient statistic for the
group's contribution to cross-group constraint violation.
\end{definition}

\begin{definition}[Macro-Violation]\label{def:macro-violation}
Given the conditional equilibria
$\{\nu_{s'}(\cdot \mid \tilde{\omega}_{s'})\}_{s' \in S'}$ from
\cref{def:timescale}, the \emph{macro-violation} of a macro-configuration
...
\begin{equation}\label{eq:macro-violation}
  ...
  \mathbb{E}_{\nu_{s'_1}(\cdot\,|\,\tilde{\omega}_{s'_1}) \otimes
  \nu_{s'_2}(\cdot\,|\,\tilde{\omega}_{s'_2})}
  \bigl[v_{\{s_1,s_2\}}(x_{s_1}, x_{s_2})\bigr],
\end{equation}
and the total macro-violation is
$\widetilde{\Viol}(\tilde{\omega}) = \sum_{e' \in E'}
\tilde{v}_{e'}$. By macro-sufficiency, this is well-defined as a
function on $\tilde{\Omega}$.
\end{definition}
...
$\nu_{s'}(\cdot \mid \tilde{\omega}_{s'})$, the remaining violation is
purely inter-group: $\Viol \approx \Viol_{\text{inter}}$. By
macro-sufficiency, $\Viol_{\text{inter}}$ under local equilibrium depends on
the micro-state only through the macro-state
...
This maps onto the DCR triad with structural precision:
...
\begin{enumerate}[nosep]
  ...
  \item Fundamental physical processes satisfy the DCR axioms: rigorously
        ...
        of distributed constraint resolution.
...
and the selection, the transaction and the adapted species: one process,
many substrates.
